...
public class TimePicker2View extends BasePickerView implements View.OnClickListener {
    ...
    private void initWheelTime(LinearLayout timePickerView) {
        wheelTime = new Wheel2Time(timePickerView, mPickerOptions.type, mPickerOptions.textGravity, mPickerOptions.textSizeContent);
        if (mPickerOptions.timeSelectChangeListener != null) {
            wheelTime.setSelectChangeCallback(new ISelectTimeCallback() {
                @Override
                public void onTimeSelectChanged() {
                    try {
                        Date date = Wheel2Time.dateFormat.parse(wheelTime.getTime());
        ...
        wheelTime.setTypeface_select(mPickerOptions.font_select);
    ...
    public void setLunarCalendar(boolean lunar) {
        try {
            int year, month, day, hours, minute, seconds;
            Calendar calendar = Calendar.getInstance();
            calendar.setTime(Wheel2Time.dateFormat.parse(wheelTime.getTime()));
            year = calendar.get(Calendar.YEAR);
            month = calendar.get(Calendar.MONTH);
            day = calendar.get(Calendar.DAY_OF_MONTH);
            hours = calendar.get(Calendar.HOUR_OF_DAY);
